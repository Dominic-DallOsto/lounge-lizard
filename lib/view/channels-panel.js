...
class ChannelsPanel {
  constructor(mainWindow) {
    ...
    this.view.setBackgroundColor('#252526')
    ...
    this.threadsTitle = new ChannelTitle('Threads', this)
    this.threadsTitle.view.setStyle({marginLeft: 10})
    this.threadsTitle.view.onMouseUp = () => mainWindow.showThreads(this.account)

    this.channelTitle = new ChannelTitle('Channels', this)
    this.channelTitle.view.setStyle({marginLeft: 10, marginTop: 15})
    this.channelTitle.view.onMouseUp = () => mainWindow.showAllChannels(this.account)

    this.dmsTitle = new ChannelTitle('Direct Messages', this)
  ...
  loadChannels() {
    ...
    if (this.account.channels.length === 0)
      return
    this.channelsList.addChildView(this.threadsTitle.view)
  ...
  selectChannelTitle(title){
    if (title == this.selectedChannelItem)
      return
    if(this.selectedChannelItem)
      this.selectedChannelItem.deselect()
    this.selectedChannelItem = title
    if(title)
      title.select()
    this.account.currentChannelId = null
  }
